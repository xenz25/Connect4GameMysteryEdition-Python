...
class XEN():
    ...
    def SHOW_FADE_TO_XEN(self,n):
        image.XEN.set_alpha(n)
        BLIT(image.XEN,image.ORIGIN)


    def xen_event_handler(self):
    ...
    def start_XEN(self):
        self.counter = 0
        while scenes.scene == 'XEN':
            if self.counter < 300:
                self.counter +=6
            self.SHOW_FADE_TO_XEN(self.counter)

            self.xen_event_handler()
            self.cursor_toggle()
