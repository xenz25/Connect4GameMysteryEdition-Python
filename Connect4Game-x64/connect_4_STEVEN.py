...
class STEVEN():
    ...
    def SHOW_FADE_TO_STEVEN(self,n):
        image.STEVEN.set_alpha(n)
        BLIT(image.STEVEN,image.ORIGIN)

    def steven_event_handler(self):
    ...
    def start_STEVEN(self):
        self.counter = 0
        while scenes.scene == 'STEVEN':
            if self.counter < 300:
                self.counter +=6
            self.SHOW_FADE_TO_STEVEN(self.counter)

            self.steven_event_handler()
            self.cursor_toggle()
